...
<?php
if(isset($_GET['type']) && isset($_GET['account1']) && isset($_GET['amount'])){
	$type = $_GET['type'];
	$amount = (int)$_GET['amount'];
	switch($type){
		case 'deposit':
			do_bank_action("000000000000", $_GET['account1'], ($amount * -1), $type);
			break;
		case 'withdraw':
			do_bank_action($_GET['account1'], "000000000000", ($amount * -1), $type);
			break;
		case 'transfer':
			//TODO figure it out
			break;
